...
class DataTransformServices:
    ...
    def data_csv_file(self, file_path):
        ...
        def sql_create_table_and_insert_data(create_table, insert_data, csv_reader, log_sql_create_table, log_sql_insert_data):
                logging.debug(f'{log_sql_create_table}')
                cursor.execute(create_table)
                logging.debug(f'{log_sql_insert_data}')
                cursor.executemany(insert_data, csv_reader)

        def open_csv_and_pull_data():
            with open(file_path, 'r', newline = '', encoding = 'latin-1') as file:
                ...
                try:
                    sql_create_table_and_insert_data(sql_create_table, sql_insert_data, csv_reader, sql_create_table, sql_insert_data)
                    print("encode_type = 'latin-1'")
                    
                except:
                    print("Os dados não estavam no tipo latin-1")
                    encode_type = detect_encoding(file_path)

                    if encode_type != 'latin-1':
                        print("encode_type != 'latin-1'")
                        convert_encoding(file_path, file_path, encode_type, 'latin-1')
